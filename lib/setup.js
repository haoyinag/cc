...
function install(options = {}) {
  ...
  const { data: loadedConfig } = config.loadConfig({ configDir, logger });
  const resolvedRcFiles = config.resolveRcFiles({ config: loadedConfig, defaultRcFiles: rcFiles });
  const enabledAssets = config.resolveEnabledAssets({ manifest, config: loadedConfig, logger });

  if (!loadedConfig) {
    const defaultConfig = config.createDefaultConfig(manifest);
    config.saveConfig(defaultConfig, { configDir });
  }
