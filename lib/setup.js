...
function install(options = {}) {
  ...
  if (!loadResult.valid) {
    const backupNote = loadResult.backup ? `（备份位置: ${loadResult.backup}）` : '';
    logger.warn(`[cmdcc] 当前配置无法解析，已使用默认配置继续执行${backupNote ? ' ' + backupNote : ''}。`);
  ...
  const normaliseResult = config.normaliseConfig(effectiveConfig, { manifest });
  if (normaliseResult.mutated && loadResult.valid) {
    config.saveConfig(effectiveConfig, { configDir });
  }
