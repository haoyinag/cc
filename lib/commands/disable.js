...
module.exports = {
  ...
  handler(ctx) {
    ...
    if (!enabledSet.has(asset.id)) {
      ctx.logger.log(`[cmdcc] 资产 ${asset.id} 已处于禁用状态。`);
      return;
    }
    ...
    ctx.logger.log(`[cmdcc] 已禁用资产: ${asset.id}`);
