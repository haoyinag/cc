...
function normaliseConfig(configData, { manifest = assetRegistry.list() } = {}) {
  if (!configData || typeof configData !== 'object') {
    return { config: configData, mutated: false };
  }
  ...
  const modules = ensureModulesContainer(configData);
  const legacySet = Array.isArray(configData.enabledAssets)
    ? new Set(configData.enabledAssets.map((value) => String(value)))
    : null;
  ...
  for (const entry of manifest) {
    ...
    if (existing !== undefined) {
      enabled = readModuleEnabled(modules, moduleId, fallback);
    } else if (legacySet) {
      enabled = legacySet.has(moduleId);
    }
  ...
  const derivedEnabled = getEnabledModuleIds(modules, manifest);
  if (!Array.isArray(configData.enabledAssets) || !arraysEqual(configData.enabledAssets, derivedEnabled)) {
    configData.enabledAssets = derivedEnabled;
    mutated = true;
  }

  return { config: configData, mutated };
}
